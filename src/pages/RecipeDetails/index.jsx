...
const DetalhesComida = () => {
  ...
  const filtersKey = Object.keys(dataDetail.meals[0]).filter(
    (key) => key.includes('strIngredient') && dataDetail.meals[0][key] !== '',
  );
    ...
    <div>
      ...
      {filtersKey.map((filter, index) => (
        <p data-testid={`${index}-ingredient-name-and-measure`}>
          {dataDetail.meals[0][filter]} -{' '}
          {dataDetail.meals[0][`strMeasure${index + 1}`]}
        </p>
      ))}
    </div>
  );
